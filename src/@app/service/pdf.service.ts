...
export class PdfService {
  ...
  public createOrderReceiptV2 = async (bookingId: string, res: Response): Promise<any> => {
    ...
    const contactY = locationY + pdf.heightOfString(location) + 1;
    const emailY = contactY + pdf.heightOfString(`Contact: ${contact}`) + 1;
    ...
    pdf.text(`Contact: ${contact}`, contentX, contactY, {align : "center"});
    pdf.text(`Email: ${email}`, contentX, emailY, {align : "center"});

    // Add a horizontal line below the email address
    const lineWidth = pdf.page.width - contentX * 2; // Calculate the line width
    const lineY = emailY + pdf.heightOfString(`Email: ${email}`) + 20; // Adjust the Y position as needed
    ...
    const receiptNo = 'Receipt No: 001223';
    const patientID = 'Patient ID: 1214';
    const visitID = 'Visit ID: 0125';
    ...
    const receiptDate = 'Receipt Date: Oct 3, 2023';
    const paymentMode = 'Payment Mode: Cash';
    const paymentStatus = 'Payment status: '+ bookingDetails.txStatus;
    ...
    const tableWidth = 600; // Adjust the width of the table as needed
    ...
    const columnWidths = [50, 300, 100, 150]; // Adjust the widths as needed
    ...
    pdf.text('Print Date Time: Oct 4, 2023',{ align: "left" });
    pdf.font('Helvetica-Bold');
    pdf.y = pdf.y - 10 ;
    const total = 'Total:  1,075.00';
    ...
    pdf.x = 100;
    pdf.text('Subjected to Indore Jurisdiction',{align:"left"});
    pdf.y = pdf.y - 10 ;
    const totalWords = "[IN WORDS [ONE THOUSAND SEVENTY FIVE ONLY]"
    ...
    pdf.moveDown();
    const hospitalName = 'For JP Hospital - Sheru Verma';
    ...
    pdf.x = 100;
    pdf.text('Note:');
    pdf.text('1. Culture Report: After 2-3 days, Enquire');
    pdf.text('2. Outside Report: No same day guarantee, Enquire');
