import { OrgOrderNoDto, PRODUCT_STATUS, ProductVo } from "aayam-clinic-core";
...
export class ProductService {
  ...
  public addUpdateProduct = async (
    ...
    try {
      if (product._id) {
          ...
          await new MetaOrgService().updateOrderNo(product.orgId, nextProductNo);
          product.del = false;
  ...
  /* ************************************* Private Methods ******************************************** */
  private _getNextProductNo = async (product: ProductVo): Promise<OrgOrderNoDto> => {
    const nextProductNo = {} as OrgOrderNoDto;
    const lastProductOrder = await new MetaOrgService().getLastOrderNo(product.orgId);
